 ...
 */
static gboolean link_camera(GstElement *e1, GstElement *e2){
  gboolean link_ok;
  GstCaps *caps;
  // set capabilities
      ...
      char *argv[])
{
  ...
  if (argc != 2) {
    g_printerr ("Usage: %s <video input>\n", argv[0]);
    return -1;
  }
  ...
  sink   = gst_element_factory_make ("appsink",       "app-data");
  ...
  // set video device to read from
  g_object_set (G_OBJECT (source), "device", argv[1], NULL);
  // enable callbacks for app sink
  ...
  // sink data to nothing, register a callback
  g_signal_connect(sink, "new-sample", G_CALLBACK(new_sample), 0);
