...
/* appsink buffer receiver as described in the documentation: https://gstreamer.freedesktop.org/documentation/tutorials/basic/short-cutting-the-pipeline.html?gi-language=c */
...
int main (int argc, char *argv[])
{
  ...
  /*  Implement: gst-launch-1.0 -v v4l2src device=/dev/video0 ! videoscale ! video/x-raw,width=160,height=120,framerate=30/1 ! appsink
  ...
  */
  ...
  filesink    = gst_element_factory_make ("appsink",   "data");
  ...
  g_object_set (G_OBJECT (filesink), "emit-signals", TRUE, NULL);
  ...
  g_signal_connect(filesink, "new-sample", G_CALLBACK(new_sample), 0);
